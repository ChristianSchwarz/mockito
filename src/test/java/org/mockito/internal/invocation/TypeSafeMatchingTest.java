...
public class TypeSafeMatchingTest {
	...
	@Test
	public void lambdaTypeMismatch() {
		final AtomicBoolean wasCalled = new AtomicBoolean();

		ArgumentMatcher<Integer> matcher = i -> {
			wasCalled.set(true);
			return true;
		};

		matchesTypeSafe().apply(matcher, "abc");
		assertThat(wasCalled.get()).isFalse();

	}
	
	@Test
	public void lambdaTypeMatch() {
		final AtomicBoolean wasCalled = new AtomicBoolean();
	

		ArgumentMatcher<String> matcher = i -> {
			wasCalled.set(true);
			return true;
		};
